...
### Phase 1 — Read the design fully *before* writing any code

Common failure mode: read one Figma frame, assume the others are just scaled versions, start coding. They are not. Each frame is a separate breakpoint *mode* with its own token values.
...
4. **Fetch asset URLs from `get_design_context`, and retry on failure.** The Figma MCP serves assets at a localhost URL with a content hash. First-attempt 404s are common (the hash refreshes between metadata calls); re-call `get_design_context` to get the current hash, then re-`curl`. Don't fall back to SVG / placeholder if the real raster is just one retry away.
5. **Record the `layout-columns` / `layout-gutter` token values per band.** If the design system specifies an N-column grid for the active mode, the implementation must use `grid-template-columns: repeat(N, ...)` with explicit `grid-column: span K` for each layer — **not** a `Nfr Mfr` approximation. The two render differently once gutters enter the math, and at pixel-match fidelity that matters.
...
- [ ] Each Figma frame's variables were read separately (Phase 1).
- [ ] User confirmed the Phase 2 artifact.
- [ ] Every band tested in the DOM with `getComputedStyle` + `getBoundingClientRect` (Phase 3).
