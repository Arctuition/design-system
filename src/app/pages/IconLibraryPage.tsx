...
// Persist the user's "Group by" choice in a cookie so it survives reloads and
// navigation. Default is "dateModified" (most-recently-changed first) — the
// most useful default for a library that's actively being added to.
const GROUP_BY_COOKIE = "ds-icon-group-by";

function readGroupByCookie(): GroupByMode {
  if (typeof document === "undefined") return "dateModified";
  const m = document.cookie.match(/(?:^|;\s*)ds-icon-group-by=([^;]+)/);
  const v = m?.[1];
...
export function IconLibraryPage() {
  ...
  const handleGroupByChange = (v: GroupByMode) => {
    setGroupBy(v);
    writeGroupByCookie(v);
  };
    ...
    return (
      <div
        ...
        className={`flex flex-col items-center justify-center gap-2 p-4 pb-2 align-bottom rounded-[var(--radius-card)] transition-all cursor-pointer ${
          selectedIcon?.id === icon.id ? "ring-1 ring-primary" : ""
        }`}
        style={{
          minHeight: 140,
  ...
  const activeGroups =
    groupBy === "size" ? groupedIcons : groupBy === "date" ? groupedByDate : groupedByDateModified;
      ...
      <div className="sticky top-0 z-10 bg-background border-b border-border" style={{ padding: "var(--spacing-4, 16px) var(--spacing-8, 32px)" }}>
        <div className="max-w-[1400px] mx-auto space-y-3">
          <div className="flex items-center gap-3">
            ...
            >
              <ArrowLeft className="size-4" /> Iconology
            </Link>
      ...
      <div className="flex-1 overflow-auto">
        <div className="max-w-[1400px] mx-auto" style={{ padding: "var(--spacing-10, 40px) var(--spacing-8, 32px)" }}>
          <div>
            ...
            {[...activeGroups.entries()].map(([label, groupIcons]) => (
              <div key={label} className="mb-10">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8" style={{ gap: 16 }}>
                  {groupIcons.map((icon) => renderIconCard(icon))}
                </div>
              </div>
            ))}
