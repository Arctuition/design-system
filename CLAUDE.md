...
| **Specialised — reference MDs** | (canonical: KV `tokenDocs` via curl, **not** `tokens/*.md`) | Editing the four reference MDs — color / size / typography / **iconology**. See §"Token reference MDs" below |
...
| Path | Purpose |
|------|---------|
| `src/app/pages/cms/IconEditor.tsx` | Icon Manager — upload (single / bulk SVG / ZIP / folder), pre-upload preview, deduplication |
...
- **`public/tokens/bootstrap.css`** is generated by the same script. In **production builds it's a 1-line `@import` shim** forwarding to the live Supabase Storage copy the CMS publishes (decision #12 — instant token publish); in dev it's the full inlined stylesheet from repo JSON. To change token *values*, edit via the CMS (a Save auto-publishes — decision #14) — not this file. To change the template, edit `_shared/token-generators.mjs`.
- **`public/tokens/*.md`** + **`breakpoints.js`** are static back-compat copies for the stable site URLs; the **live canonical** that `llms.txt` points agents at is the Supabase Storage copy (regenerated on CMS Publish). Edit token-doc content via the CMS, not these files. This set now includes **`iconology.md`** (the icon naming/sizing spec, KV slot `tokenDocs.iconology`, rendered on `/iconology`) — it rides the exact same MD-canonical pipeline as the token docs. The searchable icon grid moved to **`/iconology/library`** (separate page).
- **`public/icons.json`, `public/icons.index.json`, `public/icons/*`** are generated (gitignored, PR #50) by `scripts/generate-icons-json.mjs` + `generate-icon-files.mjs` on every `prebuild` / `predev`. Since **decision #13** these are only a back-compat/offline snapshot — the **live canonical** that `llms.txt` points agents at is the edge function (`GET /icons.index.json`, `/icons.json`, `/icons/:fileName`), which reads the `icons` KV slot per request. So an icon uploaded in the CMS reaches agents within ~60s, **no rebuild and no publish step** (unlike tokens). Both paths build the manifest via the shared `supabase/functions/_shared/icon-manifest.mjs` — change the manifest shape there, not in either script.

## Token reference MDs — canonical = KV, NOT repo

> 🔄 **CURRENT (see `.claude/decisions.md` #12, which reverses #7).**
> KV is canonical; the CMS publishes it **live to Supabase Storage**, and
> `llms.txt` points AI agents straight at those Storage URLs. So:
> - **Editing via the CMS is enough and instant** — a token **Save auto-publishes**: the `PUT /state/*` write regenerates the artifacts and mirrors KV → Storage, reaching AI agents and prototypes within seconds, no rebuild / PR / repo edit. The "Publish to Production" button still works as a manual force-refresh (decision #14).
> - The stable `https://design-system.arcsite.com/tokens/bootstrap.css` is a **shim** `@import`-ing the live Storage copy; `tokens-*.md` + `breakpoints.js` are linked directly to Storage in `llms.txt`.
> - Repo `tokens/*` is only the dev/offline + byte-identical-test source now.
> - **Egress caveat:** agents/prototypes fetch `*.supabase.co` — behind a network egress allowlist that excludes Supabase this fails; ask the admin to allowlist `dnfzdqyiepjzqrigpvzw.supabase.co` (the accepted trade-off of instant publish — see #12).

⚠️ **Historical framing — still true for the runtime CMS / public website:** `tokens/tokens-color.md`, `tokens/tokens-size-space.md`, `tokens/tokens-typography.md` are NOT the source of truth. They're a seed.

The canonical for the **public website + CMS + AI agents** lives in KV (`tokenDocs.{color,size,typography,iconology}`) and is published to Supabase Storage by the same `/design-tokens/publish` endpoint that publishes `bootstrap.css` (color/size/typography → `tokens-*.md`; iconology → `iconology.md`, rendered on `/iconology`). `public/llms.txt` points AI agents at those Storage URLs. `<ref>` below is the Supabase project ref (`VITE_SUPABASE_PROJECT_ID` in `.env` — currently `dnfzdqyiepjzqrigpvzw`; see decisions.md #10):
...
1. **Via the CMS** (preferred when the change is human-authored): open `/cms`, navigate to the Markdown editor for the target doc, edit, and Save (writes KV via `setTokenDoc`). The Save **auto-publishes** to Storage (decision #14); the "Publish to Production" button is now only a manual force-refresh.
2. **Via curl** (for scripted / batch updates):
   ...
   # Write new (full {color,size,typography,iconology} payload)
   curl -X PUT "$BASE/state/tokenDocs" -H "Authorization: Bearer $ANON" \
     -H "Content-Type: application/json" --data '{"value":{"color":"...","size":"...","typography":"...","iconology":"..."}}'
   # The PUT above already auto-publishes to Storage (decision #14). This
   # explicit mirror is now only a manual force-refresh — usually unnecessary:
   curl -X POST "$BASE/design-tokens/publish" -H "Authorization: Bearer $ANON"
   ```

Then optionally snapshot the new content back into `tokens/*.md` for repo history (a PR is OK as a *snapshot*, but the PR is not what makes the change live — the KV PUT is, since it auto-publishes).
