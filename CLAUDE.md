...
- **`public/tokens/bootstrap.css`** + **`public/tokens/breakpoints.js`** + **`public/tokens/*.md`** are generated by the same script from **live Supabase KV** token state (decision #11), falling back to committed `tokens/*.json` + `tokens/*.md` when Supabase is unreachable or `BUILD_FROM_REPO=1`. To change token *values/content*, edit via the CMS (KV) — not these output files, and not the repo fallback (which only seeds offline builds). To change the bootstrap template/structure, edit `scripts/generate-llms-txt.mjs` + `_shared/token-generators.mjs`.

## Token reference MDs — canonical = KV, NOT repo

> 🔄 **CURRENT (see `.claude/decisions.md` #11, which supersedes #7's repo-source note).**
> KV is the single source again. The prebuild fetches token state + `tokenDocs`
> from **live Supabase KV** and generates the static
> `https://design-system.arcsite.com/tokens/{bootstrap.css,tokens-*.md}` from it;
> `llms.txt` points agents at those static files. So:
> - **Editing via the CMS (KV) is enough** — a CMS edit + "Publish to Production" reaches AI agents and prototypes on the next site build (~1–2 min; Publish triggers an Amplify rebuild). No repo edit, no PR needed for token content.
> - Repo `tokens/*.json` + `tokens/*.md` are now an **offline build fallback** only (used if Supabase is unreachable, or `BUILD_FROM_REPO=1`). They can lag KV; re-sync them occasionally so the fallback isn't stale.
> - Still served as **static files from `design-system.arcsite.com`** (egress-clean), NOT live from Supabase Storage — that part of #7 stands.
