...
- **`public/tokens/*.md`** + **`breakpoints.js`** are static back-compat copies for the stable site URLs; the **live canonical** that `llms.txt` points agents at is the Supabase Storage copy (regenerated on CMS Publish). Edit token-doc content via the CMS, not these files.
...
The canonical for the **public website + CMS + AI agents** lives in KV (`tokenDocs.{color,size,typography}`) and is published to Supabase Storage by the same `/design-tokens/publish` endpoint that publishes `bootstrap.css`. `public/llms.txt` points AI agents at those Storage URLs. `<ref>` below is the Supabase project ref (`VITE_SUPABASE_PROJECT_ID` in `.env` — currently `dnfzdqyiepjzqrigpvzw`; see decisions.md #10):
